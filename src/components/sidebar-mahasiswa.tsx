...
export function SidebarMahasiswa() {
  ...
  const links = [
    { name: "Beranda Mahasiswa", icon: Home, href: "/mahasiswa" },
    { name: "Mata Kuliah Saya", icon: BookOpen, href: "/mahasiswa/kelas" },
    { name: "Daftar Tugas", icon: ClipboardList, href: "/mahasiswa/tugas" },
    { name: "Nilai & Evaluasi", icon: Award, href: "/mahasiswa/nilai" },
    { name: "Notifikasi Saya", icon: Inbox, href: "/mahasiswa/notifikasi" },
    { name: "Telegram Bot", icon: Send, href: "/mahasiswa/telegram" },
  ]
      ...
      <div className="space-y-6">
        ...
        <nav className="space-y-1">
          ...
          {links.map((link) => {
            ...
            const isActive = pathname === link.href || (link.href !== "/mahasiswa" && pathname?.startsWith(link.href))
            return (
              <Link
                key={link.name}
                href={link.href}
