...
import { getMahasiswaNotifications, markAllMahasiswaNotificationsAsRead } from "../actions"
import { useEffect, useState } from "react"
import { motion } from "framer-motion"
import { Bell, Check, CheckCircle2, Inbox, Calendar, AlertCircle } from "lucide-react"
...
export default function MahasiswaNotificationsPage() {
  ...
  const handleMarkAllRead = async () => {
    try {
      await markAllMahasiswaNotificationsAsRead()
      ...
      <DashboardLayout>
        <div className="flex h-[60vh] items-center justify-center">
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-xs font-semibold text-slate-450">Memuat log notifikasi Anda...</p>
          </div>
        </div>
      </DashboardLayout>
    )
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-450">
              Riwayat notifikasi tugas kuliah, lembar penilaian dari dosen, catatan perbaikan, serta pengumuman.
            </p>
          </div>
        ...
        {notifications.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 bg-white border border-slate-100 rounded-xl dark:bg-slate-900/40 dark:border-slate-850">
            ...
            <p className="text-[10px] text-slate-400">Log riwayat notifikasi akademik Anda masih kosong.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {notifications.map((notif, index) => {
              // Decide icon based on content
              const isAlert = notif.judul.includes("DEADLINE") || notif.judul.includes("PERLU REVISI") || notif.judul.includes("⚠️")
              const isGrade = notif.judul.includes("PENILAIAN") || notif.judul.includes("NILAI")
