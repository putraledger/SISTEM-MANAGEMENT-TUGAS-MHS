...
import { getAdminNotifications, markAllAdminNotificationsAsRead } from "../actions"
...
export default function AdminNotificationsPage() {
  ...
  const handleMarkAllRead = async () => {
    try {
      ...
      // Dispatch custom event to notify DashboardLayout to refresh the count
      ...
      <DashboardLayout>
        <div className="flex h-[60vh] items-center justify-center">
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-xs font-semibold text-slate-450">Memuat log notifikasi sistem...</p>
          </div>
        </div>
      </DashboardLayout>
    )
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <h3 className="text-base font-bold text-slate-800 dark:text-white flex items-center gap-2">
              <Bell className="size-5 text-blue-650 dark:text-blue-400" />
              Notifikasi Saya
            </h3>
            <p className="text-xs text-slate-450">
              Riwayat log notifikasi sistem dan audit yang dikirimkan untuk administrasi SIMATU.
            </p>
          </div>
        ...
        {notifications.length === 0 ? (
          ...
          <div className="space-y-3">
            {notifications.map((notif, index) => {
              const isAlert = notif.judul.includes("⚠️") || notif.judul.includes("ERROR") || notif.judul.includes("CRITICAL")
